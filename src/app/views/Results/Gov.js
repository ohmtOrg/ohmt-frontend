...
const GovRes = props => {
  const { gov,performance,AddGovFeedback,className, ...rest }= props
  const classes = useStyles();
  const theme = useTheme();
  const [open, setOpen] = React.useState(false);
  const [feedback, setfeedback] = React.useState(performance.govfeedback);
...
setfeedback(e.target.value)
  ...
  const  handleClose=()=> {
    setfeedback(performance.govfeedback)
    setOpen(false);
  }
  const handleSubmit =()=>{
    AddGovFeedback(feedback)
